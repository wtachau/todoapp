...
export const load: PageServerLoad = async ({ params, locals }) => {
  ...
  const generators = project.generators.map((g) => {
    ...
    if (g.assignmentMode === "fixed") {
      nextAssigneeName = firstName(
        g.fixedAssignee?.name,
        g.fixedAssignee?.email,
      );
    } else {
...
export const actions: Actions = {
  createTask: async ({ request, params, locals }) => {
    ...
    const isValidAssignee = project.team.members.some(
      (m) => m.userId === assignedTo,
    );
    if (!isValidAssignee) return fail(400, { error: "Invalid assignee" });

    await prisma.task.create({
      data: { title, projectId: params.id, assignedTo },
    });
  },
  ...
  createGenerator: async ({ request, params, locals }) => {
    ...
    const result = await createGeneratorFromFormData(form, params.id, userId);
  ...
  updateGenerator: async ({ request, locals }) => {
    ...
    if (!advanced && days.length === 0)
      return fail(400, { error: "Select at least one day" });
    ...
    const isMember = generator.project.team.members.some(
      (m) => m.userId === userId,
    );
    if (!isMember) return fail(403, { error: "Forbidden" });
    ...
    if (advanced) {
      try {
        ...
        nextRunAt = startOfDayInTimezone(
          rule.after(new Date()) ?? new Date(),
          DEFAULT_TIMEZONE,
        );
      } catch {
    ...
    if (assignmentMode === "round_robin" && startsWithId) {
      const other = generator.project.team.members.find(
        (m) => m.userId !== startsWithId,
      );
