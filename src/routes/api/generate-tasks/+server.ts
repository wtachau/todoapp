...
export const GET: RequestHandler = async ({ request }) => {
  ...
  return new Response(JSON.stringify({ now, generators }, null, 2), {
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  for (const gen of generators) {
    ...
    const nextRunAt = startOfDayInTimezone(
      rule.after(dayAfter, true) ?? dayAfter,
      DEFAULT_TIMEZONE,
    );
    ...
    await prisma.taskGenerator.update({
      ...
      data: {
        nextRunAt,
        lastAssignedTo:
          gen.assignmentMode === "round_robin"
            ? assignedTo
            : gen.lastAssignedTo,
      },
    });
