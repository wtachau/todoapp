<script lang="ts">
	import { taskAge } from '$lib/taskUtils';

	let { createdAt, color }: { createdAt: Date | string; color: string } = $props();
</script>

<div class="relative group shrink-0 flex items-center justify-center">
	<div class="w-2.5 h-2.5 rounded-full cursor-default" style="background-color: {color}"></div>
